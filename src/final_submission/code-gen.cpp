...
void gen_pass1(AST * node, int direction){
    if (!direction) {
        ...
        if (node->token_id == "globalVarDecl"){
            if (node->get_children()->at(0)->token_id == "int"){
                ...
                var_addresses[node->get_children()->at(1)->data["attr"]] = label;
            } else if (node->get_children()->at(0)->token_id == "boolean") {
                string label = "G" + to_string(global_counter);
...
void gen_pass2(AST * node, int direction){
    // cout << node->token_id << " direction: " << direction << endl;

    if (!direction){        
        // Direction going down.
        if (node->token_id == "mainDecl") {
            current_func = "main";
            cout << "L0: # main" << endl;
            ...
            if (node->get_children()->at(1)->token_id == "formals") {
                current_func_type = node->get_children()->at(2)->data["type"];
            } else {
            current_func_type = node->get_children()->at(1)->data["type"];
            }
            ...
            if (node->get_children()->at(1)->token_id == "formals") {
                int formal_count = 0;
                for (auto const &formal : *node->get_children()->at(1)->get_children()) {
                    string id = formal->get_children()->at(1)->data["attr"];
            ...
            string reg = allocreg();
            cout << "  li " << reg << "," << node->data["attr"] << endl;
            node->reg = reg;
            ...
            cout << start_label << ": # " << node->token_id << " start" << endl;

        } else if (node->token_id == "if") {
            ...
            for (auto a : *node->get_children()) {
                a->parent_id = node->token_id;
            }
        }
    } else {
        // Direction going up.
        if (node->token_id == "mainDecl") {
            ...
            current_func = "";
            current_func_type = "";
        } else if (node->token_id == "-"){
            cout << "L" << label_counter << ": # " << node->token_id << endl;
            label_counter++;

            string op_reg = allocreg();
            
            if (node->get_children()->size() == 1) {
                string reg = get_reg(node->get_children()->at(0));
                cout << "  negu " << op_reg << "," << reg << endl;
                freereg(reg);
            } else {
                string l_reg = get_reg(node->get_children()->at(0));
                string r_reg = get_reg(node->get_children()->at(1));
            ...
            cout << div_zero_skip << ": # " << node->token_id << " divzero skip" << endl;
            ...
            string l_identifier = node->get_children()->at(0)->data["attr"];
            string l_addr = get_identifier_addr(l_identifier);

            AST * r_node = node->get_children()->at(1);
            ...
            freereg(r_reg);
        } else if (node->token_id == "funcCall") {
            string func_name = node->get_children()->at(0)->data["attr"];
            if (node->get_children()->size() > 1) {
                int current_arg = 0;
                for (auto const n : *(node->get_children()->at(1)->get_children())) {
                    if (current_arg >= 4) {
                        cerr << "too many arguments in the function" << endl;
                        exit(1);
                    }
                    if (n->token_id == "string") {
                        string str_label = str_addresses[n->data["attr"]];
        ...
        } else if (node->token_id == "return") {
            if (!node->get_children()->empty()) {
                string reg = get_reg(node->get_children()->at(0));
                cout << "  move $v0," << reg << endl;
            }
            cout << "  j " << func_labels[current_func] << "CLEANUP" << endl;
        } else if (node->token_id == "<" || node->token_id == "<=" || node->token_id == ">" || node->token_id == ">=" || node->token_id == "==" || node->token_id == "!=") {
            cout << "L" << label_counter << ": # " << node->token_id << endl;
            label_counter++;
            ...
            cout << while_end.back() << ": # " << node->token_id << " end" << endl;

            while_start.pop_back();
            while_end.pop_back();

            // Free the expression node's register;
            freereg(node->get_children()->at(0)->reg);
        } else if (node->token_id == "if") {
            cout << if_end.back() << ": #" << node->token_id << " end" << endl;
            ...
            cout << ifelse_end.back() << ": #" << node->token_id << " end" << endl;
...
string get_reg(AST * node) {
    if (node->token_id == "stmtExpr") {
        ...
        string new_reg = allocreg();
        string id = node->get_children()->at(0)->get_children()->at(0)->data["attr"];
        string addr = get_identifier_addr(id);
...
void customTraverse(AST * root, void (*callback)(AST *, int)){
    ...
    if (root->token_id == "if") {
        customTraverse(root->get_children()->at(0), callback);
        string expr_reg = get_reg(root->get_children()->at(0));
        cout << "  beq " << expr_reg << ",$zero," << if_end.back() << endl;
        customTraverse(root->get_children()->at(1), callback);

    } else if (root->token_id == "ifelse") {
        customTraverse(root->get_children()->at(0), callback);
        string expr_reg = get_reg(root->get_children()->at(0));
        cout << "  beq " << expr_reg << ",$zero," << ifelse_else.back() << endl;
        customTraverse(root->get_children()->at(1), callback);
        cout << "  j " << ifelse_end.back() << endl;
        cout << ifelse_else.back() << ": # " << root->token_id << " else" << endl;
        customTraverse(root->get_children()->at(2), callback);

    } else if (root->token_id == "while") {
        customTraverse(root->get_children()->at(0), callback);
        string expr_reg = get_reg(root->get_children()->at(0));
        cout << "  beq " << expr_reg << ",$zero," << while_end.back() << endl;
        customTraverse(root->get_children()->at(1), callback);

    } else if (root->token_id == "&&") { 
        customTraverse(root->get_children()->at(0), callback);
        ...
        string l_reg = get_reg(root->get_children()->at(0));
        cout << "  li " << op_reg << ",0" << endl;
        cout << "  beq " << l_reg << ",$zero," << skip_label << endl;
        
        customTraverse(root->get_children()->at(1), callback);
        string r_reg = get_reg(root->get_children()->at(1));
        cout << "  beq " << r_reg << ",$zero," << skip_label << endl;
        cout << "  li " << op_reg << ",1" << endl;
        
        cout << skip_label << ": # " << root->token_id << " skip" << endl;
